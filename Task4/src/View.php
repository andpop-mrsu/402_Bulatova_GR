...
use function GulyaBulatova\guessnumber\Model\showGame;
use function GulyaBulatova\guessnumber\Model\replayGame;
use function GulyaBulatova\guessnumber\Model\commandHandler;
...
function greeting()
{
    ...
    if (!empty($user_name)) {
        echo 'Рада познакомиться, ' . $user_name . '!' . PHP_EOL . 'Давай сыграем в игру "Угадай число".'
            . ' Суть игры в том, что я загадываю число от 1 до ' . MAX_NUM .
...
function endGame($hidden_num, $attempt = false)
{
    ...
    if ($attempt) {
        ...
        echo 'Увы, ты не справился. Я загадала число: ' . $hidden_num . PHP_EOL;
        replayGame($user_name);
    }
}
